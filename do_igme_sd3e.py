...
Usage:
  input=TPM_file_name [begin=fit_range_begin end=fit_range_end epoch=number_of_epochs] do_igme_sd3e.py 
or:
...
class IntegralGME(object):
    ...
    def gradient_descent_opt(self, learning_rate=1e-6, row_rate=0.0, detail_rate=0.0, momentum=0.1, epochs=500,
        ...
        for epoch in range(epochs):
            ...
            A_eigenval, A_rightvec = np.linalg.eig(A_matrix.tolist())
            sqrtA = torch.Tensor(np.dot(np.dot(A_rightvec, np.diag(np.sqrt(A_eigenval))), np.linalg.inv(A_rightvec)))
            ...
            #for i in range(self.tau_ini, self.tau_end):
                ...
                #prediction = (torch.mm(A_matrix, T_hat_power_n) + torch.mm(T_hat_power_n, A_matrix)) / 2
                prediction = torch.mm(sqrtA, torch.mm(T_hat_power_n, sqrtA)) 
                if i >= self.tau_ini:
...
if not env_fit_input:
    print("do_igme_sd3e.py : perform IGME based on 3rd order solution of IGME")
    print("    e^(ln(A)+ln(B)) = sqrt(A) B sqrt(A)")
    print("  usage: input=TPM_file_name [begin=1 end=100 epoch=1000] python do_igme_sd3e.py")
    sys.exit()

if not os.path.exists(env_fit_input):
    print("do_igme_sd3e.py : error : cannot open `"+env_fit_input+"'")
    sys.exit()
