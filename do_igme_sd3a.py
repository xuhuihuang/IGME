...
Usage:
  input=TPM_file_name [begin=fit_range_begin end=fit_range_end epoch=number_of_epochs] do_igme_sd3a.py 
or:
...
if not env_fit_input:
    print("do_igme_sd3a.py : perform IGME based on 3rd order solution of IGME")
    print("    e^(ln(A)+ln(B)) = (AB + BA)/2")
    print("  usage: input=TPM_file_name [begin=1 end=100 epoch=1000] python do_igme_sd3a.py")
    sys.exit()

if not os.path.exists(env_fit_input):
    print("do_igme_sd3a.py : error : cannot open `"+env_fit_input+"'")
    sys.exit()
